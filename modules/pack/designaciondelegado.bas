Attribute VB_Name = "designaciondelegado"
Sub Designacion_de_Delegado()
    ...
    ' Clave general para la estructura
    Const claveGeneral As String = "PROEST2023"

    ' Variables para marcadores
    Dim lugar As String
    Dim delegado As String
    Dim cargoDelegado As String
    ...
    ' Leer el ID de la plantilla desde la celda D148
    plantillaID = wsBase.range("D148").Value
    If plantillaID = "" Then
        MsgBox "No se encontró el ID de la plantilla en la celda D148 de la hoja BBDD.", vbExclamation
        Exit Sub
    End If
    ...
    guardarRuta = Application.GetSaveAsFilename( _
        "Designacion_Delegado_Terminado.docx", "Documentos de Word (*.docx), *.docx", , "Guardar documento terminado")
    ...
    delegado = CStr(ws.range("FR2").Value)
    cargoDelegado = CStr(ws.range("GB2").Value)
    ...
    ' Proteger y ocultar la hoja "SECUENCIAS" nuevamente permitiendo modificar escenarios
    ws.Protect password:=claveSecuencias, Scenarios:=True
    ws.Visible = xlSheetHidden

    ' Definir la ruta temporal donde se descargará la plantilla
    rutaDescargaTemporal = Environ("TEMP") & "\Plantilla_DesignacionDelegado_Temp.docx"
    ...
    With wdDoc
        ...
        If .Bookmarks.Exists("Delegado") Then .Bookmarks("Delegado").range.Text = delegado
        If .Bookmarks.Exists("Cargo_delegado") Then .Bookmarks("Cargo_delegado").range.Text = cargoDelegado
