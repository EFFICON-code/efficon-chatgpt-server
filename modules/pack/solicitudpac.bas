Attribute VB_Name = "solicitudpac"
Sub Solicitud_Certificacion_PAC()
    Dim wdApp As Object
    Dim wdDoc As Object
    ...
    Dim tecnicoRequirente1 As String
    ...
    guardarRuta = Application.GetSaveAsFilename("Solicitud_Certificacion_PAC_Terminado.docx", "Documentos de Word (*.docx), *.docx", , "Guardar documento terminado")
    ...
    partida = CStr(ws.range("BU2").Value)
    presupuesto = CStr(ws.range("BV2").Value)
    tecnicoRequirente1 = CStr(ws.range("G2").Value)
    cargoTecnico1 = CStr(ws.range("H2").Value)
    ...
    With wdDoc
        ...
        If .Bookmarks.Exists("Tecnico_requirente1") Then .Bookmarks("Tecnico_requirente1").range.Text = tecnicoRequirente1
