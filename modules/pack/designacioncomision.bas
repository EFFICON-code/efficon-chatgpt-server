Attribute VB_Name = "designacioncomision"
Sub Designacion_de_Comision()
    ...
    ' Leer el ID de la plantilla desde la celda D149
    plantillaID = wsBase.range("D149").Value
    If plantillaID = "" Then
        MsgBox "No se encontró el ID de la plantilla en la celda D149 de la hoja BBDD.", vbExclamation
        Exit Sub
    End If
    ...
    ' Solicitar la ruta para guardar el documento terminado
    guardarRuta = Application.GetSaveAsFilename( _
        "Designacion_Comision_Terminado.docx", "Documentos de Word (*.docx), *.docx", , "Guardar documento terminado")
    ...
    rutaDescargaTemporal = Environ("TEMP") & "\Plantilla_DesignacionComision_Temp.docx"
